...
import * as scale from 'd3/dist/d3.min.js';
...
L.BubbleLayer = (L.Layer ? L.Layer : L.Class).extend({
  ...
  initialize: function (geojson, options) {
    ...
    let valid = this._hasRequiredProp(this.options.property)
  ...
  onAdd: function(map) {
    ...
    let geoJsonLayer = this.createLayer();
  ...
  createLayer: function(geojson) {
    ...
    let max = this._getMax(this._geojson)

    // Caluclate the minimum and maximum radius from the max area
    // TODO: how to handle zero and negative values
    let min_area = Math.PI * 3 * 3;
    let max_area = Math.PI * this.options.max_radius * this.options.max_radius;
    ...
    let property = this.options.property;
  ...
  showLegend: function(scale, max){
    ...
    let max_radius = this.options.max_radius;
    let fill = this.options.style.fillColor;
    let fill_scale = false ;
    let opacity = this.options.style.opacity;
    ...
    legend.onAdd = function(map) {
      ...
      div.innerHTML += '<strong>' + this.options.property + '</strong><br/>';
      div.style = 'background-color: #FFF; padding: 8px; font-size: 14px; text-transform: capitalize'

      for (let i = 3; i > 0; i--) {

        let area = scale(max / i / 2);
        let radius = Math.sqrt(area / Math.PI)
        let item = L.DomUtil.create('div', 'b__ubble');
  ...
  _getMax : function(geoJson) {
    let max = 0;
    let features = this._geojson.features;
    let property = this.options.property;
